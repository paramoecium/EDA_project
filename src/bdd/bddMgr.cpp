...
//    Global variables
//----------------------------------------------------------------------
BddMgr* bddMgr = new BddMgr(10, 61, 127);
...
size_t 
BddMgr::getNumCubes(const BddNode& n)
{
   size_t nv = n() & BDD_NODE_PTR_MASK;
   BddCubeNumMap::iterator mi = _numCubes.find(nv);
   if (mi == _numCubes.end()) {
      size_t nc = n.countCube();
      _numCubes.insert(pair<size_t, size_t>(nv, nc));
      return nc;
   }
   return (*mi).second;
}
...
void
BddMgr::printCacheStat() const
{
   cout << "======================" << endl;
   cout.precision(3);
   cout << "Cache Hit : " << double(_cacheHit)/_cacheAccess*100.0
        << '%' << endl;
   cout << "Cache Miss: "
        << double(_cacheAccess-_cacheHit)/_cacheAccess*100.0 << '%' << endl;
}
