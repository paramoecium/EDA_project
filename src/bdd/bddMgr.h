...
typedef map<size_t, size_t>                   BddCubeNumMap;
...
public:
   BddMgr(size_t nin = 64, size_t h = 8009, size_t c = 30011):
      _cacheAccess(0), _cacheHit(0) { init(nin, h, c); }
   ~BddMgr() { reset(); }
   ...
   // for _numCubes
   size_t getNumCubes(const BddNode&);
