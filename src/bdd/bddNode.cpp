...
BddNode
BddNode::forall(unsigned l) const
{
   if (l == 0) return (*this);

   map<size_t, size_t> forallMap;
   return forallRecur(l, forallMap);
}

BddNode
BddNode::forallRecur(unsigned l, map<size_t, size_t>& forallMap) const
{
   if (isTerminal()) return (*this);
   ...
   map<size_t, size_t>::iterator mi = forallMap.find(_nodeV);
   if (mi != forallMap.end()) return (*mi).second;

   BddNode left = getLeftCofactor(thisLevel);
   BddNode right = getRightCofactor(thisLevel);
   if (l == thisLevel) {
      BddNode res = left & right;
      forallMap[_nodeV] = res();
      return res;
   }

   // l must < getLevel()
   bool isNegEdge = false;
   BddNode t = left.forallRecur(l, forallMap);
   BddNode e = right.forallRecur(l, forallMap);
   if (t == e) {
      forallMap[_nodeV] = t();
      return t;
   }
